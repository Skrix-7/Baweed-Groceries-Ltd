...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    //Otherwise the login process begins
    else {

        //This is the SQL query to get the credentials of the supplier
        $stmt = $conn->prepare("SELECT supplierID, Fullname, Password FROM suppliers WHERE Fullname = ?");
        $stmt->bind_param("s", $fullnameInput);
        ...
        //Otherwise they are told that the full name/password was incorrect
        if (!$passwordValid && empty($errorMessage)) {
            $errorMessage = "Invalid full name or password.";
        }
    }
}
...
<head>
    ...
    <style>
        ...
        .entryFields input:focus {
            outline: none;
            border-color: #e03030;
            box-shadow: 0 0 0 2px rgba(224,48,48,0.2);
        }
